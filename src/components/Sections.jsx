		...
		<div className={`lg:flex hidden justify-around align-baseline py-5`} >
			<div className={`flex justify-center items-center`} >
				{sections.map((section) => {
					return <button className={`px-3 py-2 rounded-md mx-2 hover:bg-fayaBlue hover:text-white`} >{section}</button>
				})}
			</div>

			<div className={`flex justify-between items-center`} >
				<h1 className={`px-3 flex justify-center items-center`} >
					<FontAwesomeIcon className={`px-2 text-fayaBlue`} icon={faPhone} />
					80010808
				</h1>
